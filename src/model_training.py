...
class ModelTraining:
        ...
        try:
            ...
            def lrfn(epoch):
                if epoch<ramup_epochs:
                    ...
                    return (max_lr-start_lr) * exp_decay ** (epoch-ramup_epochs-sustain_epochs)+min_lr
            ...
            logger.info("Training model phase - model.fit() - start")
            try:
                history = model.fit(
                    x = X_train_array,
                    y = y_train,
                ...
                logger.error(str(e))
                raise CustomException("Failed to train the model - model.fit()",e)
            
            ## Create function to save model weights
            self.save_model_weights(model)
            logger.info("train_model function Completed")
    ...
    def extract_weights(self, layer_name, model):
        try:
            ...
            logger.error(str(e))
            raise CustomException("Failed to extract weights",e)
    ...
    #         logger.info("")
    #     except Exception as e:
    #         raise CustomException("Failed to ",e)
